...
func initAgentScheduleCmd() {
	...
	agentScheduleStopCmd.Flags().String("name", "", "Scheduler name to stop (default: stop all)")
...
func startSchedulerDaemon(cobraCmd *cobra.Command, info *daemon.SchedulerInfo) error {
	...
	childArgs = append(childArgs, "--engine", info.Engine)
	...
	fmt.Printf("  Logs:     %s\n", ss.LogPath())
	fmt.Printf("  Stop:     awh agent schedule stop --name %s\n", info.Name)
	return nil
}
...
func runSchedulerLoop(ctx context.Context, ss *daemon.SchedulerState, info *daemon.SchedulerInfo) {
	...
	// workerResult receives the completion signal from the goroutine managing
